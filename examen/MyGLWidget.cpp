...
{
  aspectraw = (float)w/(float)h;

  if (aspectraw<1)
  {
    fov = 2*atan(tan((M_PI)/1.5)/aspectraw);
  }
  projectTransform();
  glViewport (0, 0, w, h);  
}
...
void MyGLWidget::createBuffers ()
{
  ...
  patr1.load("./models/Patricio.obj");
  patr2.load("./models/Patricio.obj");
  patr3.load("./models/Patricio.obj");
...
void MyGLWidget::modelTransformPatricio ()
{
  ...
  TG = glm::translate(glm::mat4(1.0), glm::vec3(0.0, 0.0, ava));
...
{
  glm::mat4 TG;  // Matriu de transformació
  TG = glm::translate(glm::mat4(1.0), glm::vec3(0.0, 0.0, ava1));
  TG = glm::rotate(TG, float(M_PI/0.5), glm::vec3(0.0, 1.0, 0.0));  
  TG = glm::scale(TG, glm::vec3(escala));  
  TG = glm::translate(TG, -centreBasePatr1);  
  glUniformMatrix4fv (transLoc, 1, GL_FALSE, &TG[0][0]);
}
...
{
  glm::mat4 TG;  // Matriu de transformació
  TG = glm::translate(glm::mat4(1.0), glm::vec3(ava2, 0.0, 0.0));
  TG = glm::rotate(TG, float(M_PI/0.3), glm::vec3(0.0, 1.0, 0.0));  
  TG = glm::scale(TG, glm::vec3(escala));  
  TG = glm::translate(TG, -centreBasePatr2);  
  glUniformMatrix4fv (transLoc, 1, GL_FALSE, &TG[0][0]);
}
...
{
  glm::mat4 TG;  // Matriu de transformació
  TG = glm::translate(glm::mat4(1.0), glm::vec3(ava3, 0.0, 0.0));
  TG = glm::rotate(TG, float(M_PI/0.4), glm::vec3(0.0, 1.0, 0.0));  
  TG = glm::scale(TG, glm::vec3(escala));  
  TG = glm::translate(TG, -centreBasePatr3);  
  glUniformMatrix4fv (transLoc, 1, GL_FALSE, &TG[0][0]);
}
...
void MyGLWidget::projectTransform ()
{
  ...
  Proj = glm::perspective((double)fov, (double)aspectraw, 1.0, 60.0);
...
void MyGLWidget::calculaCapsaModel1 ()
{
  // Càlcul capsa contenidora i valors transformacions inicials
  float minx, miny, minz, maxx, maxy, maxz;
  minx = maxx = patr1.vertices()[0];
  miny = maxy = patr1.vertices()[1];
  minz = maxz = patr1.vertices()[2];
  for (unsigned int i = 3; i < patr.vertices().size(); i+=3)
  {
    if (patr1.vertices()[i+0] < minx)
      minx = patr1.vertices()[i+0];
    if (patr1.vertices()[i+0] > maxx)
      maxx = patr1.vertices()[i+0];
    if (patr1.vertices()[i+1] < miny)
      miny = patr1.vertices()[i+1];
    if (patr1.vertices()[i+1] > maxy)
      maxy = patr1.vertices()[i+1];
    if (patr1.vertices()[i+2] < minz)
      minz = patr1.vertices()[i+2];
    if (patr1.vertices()[i+2] > maxz)
      maxz = patr1.vertices()[i+2];
  }
  escala = 2.0/(maxy-miny);
  centreBasePatr1[0] = (minx+maxx)/2.0; centreBasePatr1[1] = miny; centreBasePatr1[2] = (minz+maxz)/2.0;
}
...
{// Càlcul capsa contenidora i valors transformacions inicials
  ...
  for (unsigned int i = 3; i < patr2.vertices().size(); i+=3)
  {
    ...
    if (patr2.vertices()[i+2] < minz)
      minz = patr2.vertices()[i+2];
    if (patr2.vertices()[i+2] > maxz)
      maxz = patr2.vertices()[i+2];
  }
  escala = 2.0/(maxy-miny);
  centreBasePatr2[0] = (minx+maxx)/2.0; centreBasePatr2[1] = miny; centreBasePatr2[2] = (minz+maxz)/2.0;
}
void MyGLWidget::calculaCapsaModel3 ()
{
  // Càlcul capsa contenidora i valors transformacions inicials
  float minx, miny, minz, maxx, maxy, maxz;
  minx = maxx = patr3.vertices()[0];
  miny = maxy = patr3.vertices()[1];
  minz = maxz = patr3.vertices()[2];
  for (unsigned int i = 3; i < patr3.vertices().size(); i+=3)
  {
    if (patr3.vertices()[i+0] < minx)
      minx = patr3.vertices()[i+0];
    if (patr3.vertices()[i+0] > maxx)
      maxx = patr3.vertices()[i+0];
    if (patr3.vertices()[i+1] < miny)
      miny = patr3.vertices()[i+1];
    if (patr3.vertices()[i+1] > maxy)
      maxy = patr3.vertices()[i+1];
    if (patr3.vertices()[i+2] < minz)
      minz = patr3.vertices()[i+2];
    if (patr3.vertices()[i+2] > maxz)
      maxz = patr3.vertices()[i+2];
  }
  escala = 2.0/(maxy-miny);
  centreBasePatr3[0] = (minx+maxx)/2.0; centreBasePatr3[1] = miny; centreBasePatr3[2] = (minz+maxz)/2.0;
}
